...
class Chatbot:
    ...
    def _run_action(self, retrieved_run):
        ...
        client.beta.threads.runs.submit_tool_outputs( # function calling의 결과를 다시 보냄
            thread_id = self.thread.id, 
    ...
    def get_response_content(self, run) -> tuple[openai.types.beta.threads.run.Run, str]:
        ...
        while(True):
            ...
            if retrieved_run.status == "completed":
                break
            elif retrieved_run.status == "requires_action": # function 호출이 필요하다고 판단
    ...
    def get_interpreted_code(self, run_id):
        ...
        for run_step_data in run_steps_dict['data']:
            ...
            print("step_details", step_details) #step detail 로직이 섞여있음
...
if __name__ == "__main__":
    ...
    file_paths = ["./files/deposit.json", "./files/fund.json"]
    ...
    print("assistant id: ", assistant.id, "thread id: ", thread.id)
