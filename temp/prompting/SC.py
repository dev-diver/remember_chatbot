...
def get_most_frequent_answer(template :str, iterations :int=10) -> str:
    answers :Dict[str,int] = defaultdict(int)

    for idx in range(iterations):
        response = request_to_llm(template, temperature=0.3)
        print(f"\n{idx+1}번째 샘플:")
        print(response)

        match = re.search(r"정답: (\d+분)", response)
        if match:
            parsed_answer = match.group(1)
            answers[parsed_answer] += 1\
    
    sorted_answers : list[tuple[str, int]] = sorted(answers.items(), key=lambda x: x[1], reverse=True)
    print(f"\n빈도표: {sorted_answers}")
