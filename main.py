...
def generate_thoughts(selected):
    ...
    samples = []
    message = sampling_template.format(agenda=agenda, selected=selected)
    for _ in range(5):
        sample = request_gpt(message, model3, 1.2)
        samples.append(sample['구현'])
    return samples

def evalutate(thoughts):
    values = []
    for thought in thoughts:
        message = evaluation_template.format(agenda=agenda, thought=thought)
        value = request_gpt(message, model4, 0)
        values.append({
            "thought": thought,
            "value": value
        })
        print(f"thoughts: {thought}, totalValue: {value['총점']}")
    return values
...
report = request_gpt(summary, model4, 0, type="text")
